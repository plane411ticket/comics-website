...
export default function CommentSection() {
  ...
  const [loading, setLoading] = useState(false); // Số bình luận hiển thị ban đầu
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      const response = await axios.get("/api/comments"); // API lấy toàn bộ bình luận
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      setNewComment("");
      fetchComments(); // Reload list sau khi gửi thành công
    } catch (error) {
  ...
  const handleLike = async (id: number, parentId?: number) => {
    try {
      if (parentId) {
        await axios.post(`/api/comments/${parentId}/reply/${id}/like`);
      } else {
